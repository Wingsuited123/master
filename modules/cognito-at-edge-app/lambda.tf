...
resource "archive_file" "lambda" {
  ...
  type        = "zip"
  source_dir  = each.value.is_template ? "${each.value.source_dir}/temp-${var.identifier}" : each.value.source_dir
...
resource "terraform_data" "lambda" {
  ...
  provisioner "local-exec" {
    ...
    command     = each.value.is_template ? "mkdir -p \"$TEMP_DIR\" && find \"$SOURCE_DIR\" -maxdepth 1 -type f -name \"*.*\" -exec cp {} \"$TEMP_DIR\" \\; && cd \"$TEMP_DIR\" && \"$INSTALL_CMD\" $INSTALL_ARGS && echo \"$CONFIG_CONTENT\" > $CONFIG_FILE" : "cd \"$SOURCE_DIR\" && \"$INSTALL_CMD\" $INSTALL_ARGS && echo \"$CONFIG_CONTENT\" > $CONFIG_FILE"
    environment = {
      TEMP_DIR       = "${each.value.source_dir}/temp-${var.identifier}"
...
resource "terraform_data" "lambda" {
  for_each = { for func in local.lambda_functions : func.name => func if func.is_template }
